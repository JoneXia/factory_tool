...
import com.petkit.android.widget.pulltorefresh.internal.FlipLoadingLayout;
import com.petkit.android.widget.pulltorefresh.internal.LoadingLayout;
import com.petkit.android.widget.pulltorefresh.internal.RotateLoadingLayout;
import com.petkit.android.widget.pulltorefresh.internal.TweenAnimLoadingLayout;
import com.petkit.android.widget.pulltorefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
	...
	private boolean isTempRefreshing;

	@Override
	public void setRefreshing(Mode mode) {
		isTempRefreshing = true;
		tempMode = mCurrentMode;
		mCurrentMode = mode;
		if (!isRefreshing()) {
		...
		if (doScroll) {
			if (mShowViewWhileRefreshing) {
				...
				switch (mCurrentMode) {
					...
					default:
					case PULL_FROM_START:
						smoothScrollTo((int)(-getHeaderSize() * 0.8), listener);
						break;
				}
			} else {
				smoothScrollTo(0);
	...
	public void setNeedRefreshView(boolean isNeedRefreshView) {
		this.isNeedRefreshView = isNeedRefreshView;
	}
	...
	private void callRefreshListener() {
		if (null != mOnRefreshListener) {
			...
			if(isTempRefreshing){
				mCurrentMode = tempMode;
				isTempRefreshing = false;
			}
		}
	}
...
/*	public int getMaximumPullScrollHeight(){
