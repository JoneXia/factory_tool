	...
	public ZipHexInputStream(final InputStream stream, final int mbrSize, final int types) throws IOException {
		...
		try {
			...
			while ((ze = getNextEntry()) != null) {
				...
				if (types != BLEConsts.TYPE_AUTO && (
						((softDevice || systemInit) && (types & BLEConsts.TYPE_SOFT_DEVICE) == 0) ||
								((bootloader || systemInit) && (types & BLEConsts.TYPE_BOOTLOADER) == 0) ||
						((application || applicationInit) && (types & BLEConsts.TYPE_APPLICATION) == 0))) {
					continue;
				}
