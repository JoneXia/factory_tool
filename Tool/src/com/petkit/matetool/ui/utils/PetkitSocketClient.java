...
public class PetkitSocketClient {
    ...
    public void connect(String remoteIp, int remotePort) {
        isCanceled = false;
        self.getLocalSocketClient(remoteIp, remotePort).connect();
    }
    ...
    public void disconnect() {
        ...
        data.put("key", 101);
        sendString(new Gson().toJson(data));

        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                if(self.localSocketClient != null){
                    self.localSocketClient.disconnect();
                }
            }
        }, 20);
    }
    ...
    public boolean isConnected() {
        return self.localSocketClient != null && self.localSocketClient.isConnected();
    }

    public void registerSocketClientDelegate(SocketClientDelegate socketClientDelegate) {
        self.localSocketClient.registerSocketClientDelegate(socketClientDelegate);
    }

    /* Properties */
    private SocketClient localSocketClient;
    public SocketClient getLocalSocketClient(String remoteIp, int remotePort) {
        if(localSocketClient != null && localSocketClient.getAddress() != null) {
            if(!remoteIp.equals(localSocketClient.getAddress().getRemoteIP()) ||
                    !String.valueOf(remotePort).equals(localSocketClient.getAddress().getRemotePort())) {
        ...
        if (this.localSocketClient == null) {
            ...
            localSocketClient.setAddress(new SocketClientAddress(remoteIp, remotePort));
...
//                    }
    ...
    public void sendString(String data) {
        if(localSocketClient != null) {
            localSocketClient.sendString(data);
        }
    }
    ...
    private void __i__setupConstantHeartBeat(SocketClient socketClient) {
        ...
        heartbeatData.put("key", 100);
        socketClient.getHeartBeatHelper().setDefaultSendData(CharsetUtil.stringToData(new Gson().toJson(heartbeatData), CharsetUtil.UTF_8));

        /**
         * 设置远程端发送到本地的心跳包信息内容，用于判断接收到的数据包是否是心跳包
         * 通过{@link SocketResponsePacket#isHeartBeat()} 查看数据包是否是心跳包
         */
        socketClient.getHeartBeatHelper().setDefaultReceiveData(CharsetUtil.stringToData(new Gson().toJson(heartbeatData), CharsetUtil.UTF_8));
